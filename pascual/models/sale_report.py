...
class SaleReport(models.Model):
    ...
    channelsales_id = fields.Selection(string="Canal de Ventas",
                                       selection=[('local', 'Locales'), ('autoservice', 'Autoservicio'),
                                                  ('distribuitor', 'Distribuidores'),
        ...
        select_str = """
            WITH currency_rate as (%s)
             SELECT min(l.id) as id,
                    ...
                    sum(p.volume * l.product_uom_qty / u.factor * u2.factor) as volume,partner.channelsales_id
